<?php
class DBO_Asset_Select {
	const TABLE_NAME = "asset_selects";
	const MODEL_NAME = "DBO_Asset_Select_Model";
	...
	public static function set($asset_id) {
		$isSelect = self::getOneByAsset($asset_id);
		
		if($isSelect == false) {
			$sth = Ode_DBO::getInstance()->prepare("INSERT INTO " . self::TABLE_NAME . " (asset_id) VALUES (:asset)");
			$sth->bindValue(":asset", $asset_id);
