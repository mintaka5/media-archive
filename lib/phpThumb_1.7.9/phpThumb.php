			...
			if ($result = @mysql_query($phpThumb->config_mysql_query, $cid)) {
				if ($row = @mysql_fetch_array($result)) {
					...
					//$phpThumb->setSourceData($row[0]);
                                        /**
                                         * since we are not storing binary data in database,
                                         * we must re-read the file into phpthumb.
                                         */
                                        
                                        if(Ode_Auth::getInstance()->isAuth()) {
                                            $phpThumb->setSourceData(file_get_contents(IMAGE_STORAGE_PATH.$row[0]));
                                        } else if(isset($_GET['api']) &&
                                        	Ode_Auth::getInstance()->isValidAPI(trim($_GET['api']), trim($_GET['user']))) {

                                        	$phpThumb->setSourceData(file_get_contents(IMAGE_STORAGE_PATH.$row[0]));
                                        } else {
                                            //$phpThumb->setSourceData(file_get_contents(APP_PATH."assets/images/no_image.jpg"));
                                        	$phpThumb->setSourceData(file_get_contents(IMAGE_STORAGE_PATH.$row[0]));
                                        }
                                        //var_dump(IMAGE_STORAGE_PATH.$row[0]); die();
					...
					mysql_close($cid);
                    /**
                      * Changed 2011/12/06: want to show no image file instead of phpThumb errors
                      */
 ...
 */
$allowedGETparameters[] = 'api';
$allowedGETparameters[] = 'user';
$allowedGETparameters[] = 'hid';
...
/**
 * Set watermark on if larger than 400 pixels
 */
$font_name = "Arial.ttf";
$font_color = "FFFFFF";
$font_size = $phpThumb->getParameter("w") * 0.021;
$font_opacity = 50;
$margin = $phpThumb->getParameter("w") * 0.09;

$fltr = array();

if($phpThumb->getParameter("w") >= 200 || 
        $phpThumb->getParameter("h") >= 200) {
    
    if((!isset($_GET['api']) && !Ode_Auth::getInstance()->isValidAPI(trim($_GET['api']), trim($_GET['user'])))) {
    	array_push($fltr, "wmt|". $watermark_text ."|".$font_size."|*|". $font_color ."|". $font_name ."|".$font_opacity."|" . $margin);
    	
        /*array_push($fltr, "wmt|". $watermark_text ."|".$font_size."|TL|". $font_color ."|". $font_name ."|".$font_opacity."|" . $margin);
        array_push($fltr, "wmt|". $watermark_text ."|".$font_size."|TR|". $font_color ."|". $font_name ."|".$font_opacity."|" . $margin);
        array_push($fltr, "wmt|". $watermark_text ."|".$font_size."|C|". $font_color ."|". $font_name ."|".$font_opacity."|");
        array_push($fltr, "wmt|". $watermark_text ."|".$font_size."|BL|". $font_color ."|". $font_name ."|".$font_opacity."|" . $margin);
        array_push($fltr, "wmt|". $watermark_text ."|".$font_size."|BR|". $font_color ."|". $font_name ."|".$font_opacity."|" . $margin);*/
    }
}
if(!empty($fltr)) {
...
//var_dump($phpThumb->rawImageData); die();
...
//var_dump($phpThumb->cache_filename); die();
